...
- ~~Review and source approved art for the 320 expanded roster rows that currently use the explicit text-only art fallback~~ — **closed 2026-09-20.** All 410 rows carry reviewed art; no Character uses the text-only fallback. See `tickets/MS-032-roster-art-and-onboarding.md`.
...
- Replace the explicitly labeled cross-version fallbacks when version-matched official art becomes available: all 26 Melee entries use official Smash Ultimate renders. UMVC3's fallbacks were retired on 2026-09-20.
